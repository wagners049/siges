...
class SigesNewUserForm extends TPage
{
    protected $form;
    private $formFields = [];
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            $object->login = $data->email;
            $object->active = 'N';
            $object->frontpage_id = '12';
            $object->system_unit_id = '1';
             ...
             */
            /* 
            $url = sprintf( 'id=%s&confirm=%s',$object->id, md5($object->email) );
            $prefs = SystemPreference::getAllPreferences();

            if ($prefs['send_mail_reg'] == 1)
            {
                $barra = substr($prefs['url_base'], -1) != '/' ? '/' : '';

                $link = $prefs['url_base'] . $barra . 'index.php?class=ConfirmUserRegistration&' . $url;
                
                $mail = new TMail;
                $mail->setFrom($prefs['mail_from'], 'Siges');
                $mail->setSubject('SIGES - Confirmação de Cadastro');
                $mail->setHtmlBody("Você solicitou o cadastro no sistema SIGES, por favor clique no link abaixo para concluir o cadastro e utilizar o sistema!<br><br>
                                    Usuário: $object->login <br>
                                    <a href='$link'>Validar acesso e concluir o cadastro</a><br><br>
                                    <p>Caso seu email não tenha suporte a mensagens em html, copie o link abaixo e cole no navegador.</p>
                                    <p>$link</p>
                                    Atenciosamente,<br>
                                    <b>Equipe de cadastro SIGES</b>");
