...
class BuilderTableDiffForm extends TPage
{
	...
	public function __construct($param)
	{
		try
		{
			...
			$cell = $row->addCell(new TLabel("Tabelas do banco de dados"));
	...
	{
		$items = [];
		$items[''] = 'Selecione a tabela do banco de dados';
	...
	public static function onAnalisar($param)
	{
		try
		{
			...
			{
				throw new Exception("Você precisa escolher <b>todas</b> as tabelas do banco de dados que serão alteradas", 1);
			}
			...
			$databaseMergeSession->table_name_equals  = $param['name_equals']??[];
	...
	public function onLoad($param = null)
	{
		try
		{
			...
			{
				throw new Exception("Selecione os bancos do Builder e do projeto");
			}
			...
			if(! empty($diffs['name_equals']))
			{
				foreach ($diffs['name_equals'] as $table)
				{
					...
					$checkEqual = new TCheckButton('name_equals[]');
					$checkEqual->setIndexValue($table);
					$checkEqual->setValue($table);
					$checkEqual->style = 'display: none';

					$div = new TElement('div');
					$div->add($labelBuilder);
					$div->add($checkEqual);
			...
			if(! empty($diffs['not_found']))
			{
				foreach ($diffs['not_found'] as $table)
				{
					...
					$criar->add('Criar');
					$criar->title = "Adicionar a tabela {$table} no banco de dados";
			...
			if(! empty($diffs['drops']))
			{
				foreach ($diffs['drops'] as $table)
				{
					...
					$manter->add('Manter');
					$manter->title = "Manter a tabela {$table} no banco de dados";

					$deletar = new TElement('div');
					$deletar->add('Apagar');
					$deletar->title = "Apagar a tabela {$table} e as suas chaves estrangeiras no banco de dados";
	...
	public function fireEvents()
	{
		...
		$data->{"news[]"} = $databaseMergeSession->table_news;
		$data->{"drops[]"} = $databaseMergeSession->table_drops;
		$data->{"name_equals[]"} = $databaseMergeSession->data_TableDiffForm['name_equals']??[];
